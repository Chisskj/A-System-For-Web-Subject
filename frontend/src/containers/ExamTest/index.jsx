...
const ExamTest = () => {
  ...
  const [questionInGroup, setQuestionInGroup] = useState({});
  ...
  const handleChangeAnswer = (answerId) => {
    setAnswers((prevAnswers) => ({
      ...prevAnswers,
      [questionSelected.data.id]: answerId,
    }));
  };
  ...
  const handleFinishExam = async () => {
    const examData = {
      doTime: contest.exam_time * 60 - timeDoExam,
      contestId: contest.id,
      groupQuestionId: contest.group_question,
      answers,
    };
    ...
    if (data1 && data1.status) {
      ...
      enqueueSnackbar((data1 && data1.message) || 'Mark failed', {
        variant: 'error',
      });
    }
  };
  ...
  const fetchContest = async () => {
    ...
    const data1 = JSON.parse(data);
    if (data1 && data1.status) {
      const { contest: contestData } = data1.result;
      setContest(contestData);
      const response = await apis.question.getQuestionsInGroup({
        groupQuestionId: contestData.group_question,
      });
      const jsonResponse = JSON.parse(response);
      setQuestionInGroup({ result: jsonResponse.result });
      setQuestionSelected({
        position: 0,
        data: jsonResponse.result.data[0],
      });
      setTimeDoExam(contestData.exam_time * 60);
      handleStartExam(contestData.exam_time * 60);
      setIsLoading(false);
    } else {
  ...
  const handleClickQuestion = (pos) => (e) => {
    ...
    setQuestionSelected({
      position: pos,
      data: questionInGroup.result.data[pos],
    });
  };
                  ...
                  questionSelected.data.answers
                    ...
                    .map((el, index) => (
                      <Box
                        ...
                        style={{
                          background: `${
                            answers[questionSelected.data.id] === el.answer_id
                              ? '#81d1a2'
                              : '#eceff0'
                          }`,
                          padding: '20px 20px',
                          ...
                          cursor: 'pointer', // Thêm dòng này để con trỏ chuột trở thành con trỏ tương tác
                        }}
                        className={classes.answerRow}
                        onClick={() => handleChangeAnswer(el.answer_id)}
                      >
          ...
          <Grid item xs={4}>
            <Paper className={classes.questionBox}>
              ...
              <Box className={classes.listQuestionBox}>
                {questionInGroup &&
                  questionInGroup.result.data.map((el, index) => (
                    <Button
                      key={index}
