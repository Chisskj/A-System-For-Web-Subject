...
const GroupQuestion = () => {
  ...
  const fetchGroupQuestions = async (keyword) => {
    ...
    if (res && res.status) {
      setGroupQuestions(res.result.data);
      setIsLoading(false);
    } else {
  ...
  const handleSave = (el) => async (e) => {
    ...
    if (el.id) {
      ...
      if (data1 && data1.status) {
        ...
        newGroupQuestions[pos] = { ...data1.result.groupQuestion };
        setGroupQuestions(newGroupQuestions);
        enqueueSnackbar('Update success', { variant: 'success' });
      } else {
        enqueueSnackbar((data1 && data1.message) || 'Update failed', {
          variant: 'error',
        });
      }
    } else {
      ...
      if (data1 && data1.status) {
        setOpenModal(false);
        const newGroupQuestions = [...groupQuestions];
        newGroupQuestions.push({ ...data1.result.groupQuestion });
        setGroupQuestions(newGroupQuestions);
        enqueueSnackbar('Create success', { variant: 'success' });
      } else {
        enqueueSnackbar((data1 && data1.message) || 'Create failed', {
          variant: 'error',
        });
      }
    }
